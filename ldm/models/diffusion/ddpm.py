...
class DDPM(pl.LightningModule):
    ...
    def predict_start_from_z_and_v(self, x_t, t, v):
        ...
        return extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape) * x_t - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * v

    def predict_eps_from_z_and_v(self, x_t, t, v):
        return extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape) * v + extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * x_t
    ...
    def get_v(self, x, noise, t):
        return extract_into_tensor(self.sqrt_alphas_cumprod, t, x.shape) * noise - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x.shape) * x
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self, first_stage_config, cond_stage_config, num_timesteps_cond=None, cond_stage_key="image", cond_stage_trainable=False, concat_mode=True, cond_stage_forward=None, conditioning_key=None, scale_factor=1.0, scale_by_std=False, force_null_conditioning=False, *args, **kwargs):
...
class LatentInpaintDiffusion(LatentFinetuneDiffusion):
    ...
    def log_images(self, *args, **kwargs):
        ...
        log["masked_image"] = rearrange(args[0]["masked_image"], "b h w c -> b c h w").to(memory_format=torch.contiguous_format).float()
        return log
...
class ImageEmbeddingConditionedLatentDiffusion(LatentDiffusion):
    def __init__(self, embedder_config, embedding_key="jpg", embedding_dropout=0.5, freeze_embedder=True, noise_aug_config=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, **kwargs):
        ...
        if self.training:
            c_adm = torch.bernoulli((1.0 - self.embedding_dropout) * torch.ones(c_adm.shape[0], device=c_adm.device)[:, None]) * c_adm
    ...
    def log_images(self, batch, N=8, n_row=4, **kwargs):
        log = dict()
        z, c, x, xrec, xc = self.get_input(batch, self.first_stage_key, bs=N, return_first_stage_outputs=True, return_original_cond=True)
        log["inputs"] = x
        log["reconstruction"] = xrec
        ...
        with ema_scope(f"Sampling"):
            samples_cfg, _ = self.sample_log(cond=c, batch_size=N, ddim=True, ddim_steps=kwargs.get("ddim_steps", 50), eta=kwargs.get("ddim_eta", 0.0), unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=uc_,)
            x_samples_cfg = self.decode_first_stage(samples_cfg)
