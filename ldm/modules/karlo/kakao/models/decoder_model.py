...
class Text2ImProgressiveModel(torch.nn.Module):
    ...
    def create_plm_dec_model(self):
        ...
        if self._model_conf.channel_mult == "":
            ...
            channel_mult = tuple(int(ch_mult) for ch_mult in self._model_conf.channel_mult.split(","))
    ...
    def get_sample_fn(self, timestep_respacing):
        ...
        sample_fn = diffusion.ddim_sample_loop_progressive if use_ddim else diffusion.p_sample_loop_progressive

        return sample_fn

    def forward(
        self, txt_feat, txt_feat_seq, tok, mask, img_feat=None, cf_guidance_scales=None, timestep_respacing=None,
    ):
        ...
        def guided_model_fn(x_t, ts, **kwargs):
            ...
            half_eps = uncond_eps + cf_guidance_scales.view(-1, 1, 1, 1) * (cond_eps - uncond_eps)
        ...
        sample_outputs = sample_fn(guided_model_fn, (bsz, 3, img_sz, img_sz), noise=None, device=txt_feat.device, clip_denoised=True, model_kwargs=cond,)

        for out in sample_outputs:
            sample = out["sample"]
            yield sample if cf_guidance_scales is None else sample[: sample.shape[0] // 2]
...
class Text2ImModel(Text2ImProgressiveModel):
    def forward(
        ...
        last_out = None
        for out in super().forward(txt_feat, txt_feat_seq, tok, mask, img_feat, cf_guidance_scales, timestep_respacing,):
            last_out = out
        return last_out
