...
class CustomizedCLIP(CLIP):
    ...
    def load_from_checkpoint(cls, ckpt_path: str):
        ...
        model = cls(embed_dim, image_resolution, vision_layers, vision_width, vision_patch_size, context_length, vocab_size, transformer_width, transformer_heads, transformer_layers,)
...
class CustomizedTokenizer(SimpleTokenizer):
    ...
    def padded_tokens_and_mask(self, texts, text_ctx):
        assert isinstance(texts, list) and all(isinstance(elem, str) for elem in texts), "texts should be a list of strings"

        all_tokens = [[self.sot_token] + self.encode(text) + [self.eot_token] for text in texts]

        mask = [[True] * min(text_ctx, len(tokens)) + [False] * max(text_ctx - len(tokens), 0) for tokens in all_tokens]
