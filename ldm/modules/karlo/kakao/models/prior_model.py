...
class PriorDiffusionModel(torch.nn.Module):
    ...
    def __init__(self, config, tokenizer, clip_mean, clip_std):
        ...
        self.model = PriorTransformer(text_ctx=self._model_conf.text_ctx, xf_width=self._model_conf.xf_width, xf_layers=self._model_conf.xf_layers, xf_heads=self._model_conf.xf_heads, xf_final_ln=self._model_conf.xf_final_ln, clip_dim=self._model_conf.clip_dim,)
    ...
    @classmethod
    def load_from_checkpoint(cls, config, tokenizer, clip_mean, clip_std, ckpt_path, strict: bool = True):
    ...
    def forward(
        self, txt_feat, txt_feat_seq, mask, cf_guidance_scales=None, timestep_respacing=None, denoised_fn=True,
    ):
        ...
        sample = sample_fn(guided_model_fn, (bsz_, self.model.clip_dim), noise=None, device=txt_feat.device, clip_denoised=False, denoised_fn=lambda x: torch.clamp(x, -10, 10), model_kwargs=cond,)
