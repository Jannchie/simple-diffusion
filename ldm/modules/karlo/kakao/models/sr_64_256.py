...
class ImprovedSupRes64to256ProgressiveModel(torch.nn.Module):
    ...
    def forward(self, low_res, timestep_respacing="7", **kwargs):
        assert timestep_respacing == "7", "different respacing method may work, but no guaranteed"

        sample_fn = self.get_sample_fn(timestep_respacing)
        sample_outputs = sample_fn(self.model_first_steps, self.model_last_step, shape=low_res.shape, clip_denoised=True, model_kwargs=dict(low_res=low_res), **kwargs,)
