...
class GaussianDiffusion(th.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("alphas_cumprod", th.from_numpy(alphas_cumprod), persistent=False)
        self.register_buffer("alphas_cumprod_prev", th.from_numpy(alphas_cumprod_prev), persistent=False)
        self.register_buffer("alphas_cumprod_next", th.from_numpy(alphas_cumprod_next), persistent=False)

        self.register_buffer("sqrt_alphas_cumprod", th.from_numpy(sqrt_alphas_cumprod), persistent=False)
        self.register_buffer(
            "sqrt_one_minus_alphas_cumprod", th.from_numpy(sqrt_one_minus_alphas_cumprod), persistent=False,
        )
        self.register_buffer(
            "log_one_minus_alphas_cumprod", th.from_numpy(log_one_minus_alphas_cumprod), persistent=False,
        )
        self.register_buffer(
            "sqrt_recip_alphas_cumprod", th.from_numpy(sqrt_recip_alphas_cumprod), persistent=False,
        )
        self.register_buffer(
            "sqrt_recipm1_alphas_cumprod", th.from_numpy(sqrt_recipm1_alphas_cumprod), persistent=False,
        )

        self.register_buffer("posterior_variance", th.from_numpy(posterior_variance), persistent=False)
        self.register_buffer(
            "posterior_log_variance_clipped", th.from_numpy(posterior_log_variance_clipped), persistent=False,
        )
        self.register_buffer(
            "posterior_mean_coef1", th.from_numpy(posterior_mean_coef1), persistent=False,
        )
        self.register_buffer(
            "posterior_mean_coef2", th.from_numpy(posterior_mean_coef2), persistent=False,
        )
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, **ignore_kwargs,
    ):
        """
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance, model_log_variance = {
                ...
                ModelVarType.FIXED_LARGE: (th.cat([self.posterior_variance[1][None], self.betas[1:]]), th.log(th.cat([self.posterior_variance[1][None], self.betas[1:]])),),
    ...
    def p_sample_loop(
        self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, device=None, progress=False,
    ):
        """
    ...
    def p_sample_loop_progressive_for_improved_sr(
        self, model, model_aux, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, device=None, progress=False,
    ):
        """
        ...
        for idx, i in enumerate(indices):
            t = th.tensor([i] * shape[0], device=device)
            with th.no_grad():
                out = self.p_sample(model_aux if len(indices) - 1 == idx else model, img, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn, cond_fn=cond_fn, model_kwargs=model_kwargs,)
                yield out
                img = out["sample"]
    ...
    def ddim_sample_loop(
        self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, device=None, progress=False, eta=0.0,
    ):
        """
