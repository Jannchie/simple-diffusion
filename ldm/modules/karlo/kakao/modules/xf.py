...
class PriorTransformer(nn.Module):
    ...
    def __init__(
        self, text_ctx, xf_width, xf_layers, xf_heads, xf_final_ln, clip_dim,
    ):
        super().__init__()
        ...
        self.time_embed = nn.Sequential(nn.Linear(xf_width, xf_width), nn.SiLU(), nn.Linear(xf_width, xf_width),)
        ...
        self.transformer = Transformer(text_ctx + self.ext_len, xf_width, xf_layers, xf_heads,)
        if xf_final_ln:
            self.final_ln = LayerNorm(xf_width)
        else:
            self.final_ln = None

        self.positional_embedding = nn.Parameter(th.empty(1, text_ctx + self.ext_len, xf_width))
    ...
    def forward(
        self, x, timesteps, text_emb=None, text_enc=None, mask=None, causal_mask=None,
    ):
        bsz = x.shape[0]
