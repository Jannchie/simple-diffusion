...
from .types import AnimalPoseResult, FaceResult, HandResult, HumanPoseResult
...
def encode_poses_as_json(
    ...
    def compress_keypoints(keypoints: Union[List[Keypoint], None]) -> Union[List[float], None]:
        ...
        return [value for keypoint in keypoints for value in ([float(keypoint.x), float(keypoint.y), 1.0] if keypoint is not None else [0.0, 0.0, 0.0])]
...
class OpenposeDetector:
    ...
    def detect_poses(self, oriImg, include_hand=False, include_face=False) -> List[HumanPoseResult]:
        """
        ...
        with torch.no_grad():
            ...
            for body in bodies:
                ...
                results.append(
                    HumanPoseResult(
                        BodyResult(
                            keypoints=[Keypoint(x=keypoint.x / float(W), y=keypoint.y / float(H)) if keypoint is not None else None for keypoint in body.keypoints],
