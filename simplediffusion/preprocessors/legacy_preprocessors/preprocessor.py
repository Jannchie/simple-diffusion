...
def canny(img, res=512, thr_a=100, thr_b=200, **_):
    l, h = thr_a, thr_b
...
def scribble_thr(img, res=512, **_):
    img, remove_pad = resize_image_with_pad(img, res)
...
def scribble_xdog(img, res=512, thr_a=32, **_):
    img, remove_pad = resize_image_with_pad(img, res)
...
def tile_resample(img, res=512, thr_a=1.0, **_):
    img = HWC3(img)
    if thr_a < 1.1:
        return img, True
    H, W, C = img.shape
...
def threshold(img, res=512, thr_a=127, **_):
    img, remove_pad = resize_image_with_pad(img, res)
    result = np.zeros_like(img, dtype=np.uint8)
...
def scribble_hed(img, res=512, **_):
...
def mediapipe_face(img, res=512, thr_a=10, thr_b=0.5, **_):
    max_faces = int(thr_a)
...
def mlsd(img, res=512, thr_a=0.1, thr_b=0.1, **_):
...
def depth_anything(img, res: int = 512, colored: bool = True, **_):
    img, remove_pad = resize_image_with_pad(img, res)
...
def midas(img, res=512, a=np.pi * 2.0, **_):
    img, remove_pad = resize_image_with_pad(img, res)
    global model_midas
...
def midas_normal(img, res=512, a=np.pi * 2.0, thr_a=0.4, **_):  # bg_th -> thr_a
    bg_th = thr_a
    img, remove_pad = resize_image_with_pad(img, res)
    global model_midas
    if model_midas is None:
...
def leres(img, res=512, a=np.pi * 2.0, thr_a=0, thr_b=0, boost=False, **_):
    img, remove_pad = resize_image_with_pad(img, res)
    global model_leres
...
def pidinet_safe(img, res=512, **_):
    img, remove_pad = resize_image_with_pad(img, res)
    global model_pidinet
    if model_pidinet is None:
        from .annotator.pidinet import apply_pidinet
...
def scribble_pidinet(img, res=512, **_):
...
# def clip(img, res=512, config="clip_vitl", low_vram=False, **_):
#     img = HWC3(img)
#     global clip_encoder
...
def recolor_luminance(img, res=512, thr_a=1.0, **_):
...
def recolor_intensity(img, res=512, thr_a=1.0, **_):
...
def blur_gaussian(img, res=512, thr_a=1.0, **_):
    img, remove_pad = resize_image_with_pad(img, res)
    img = remove_pad(img)
...
def anime_face_segment(img, res=512, **_):
    img, remove_pad = resize_image_with_pad(img, res)
...
#     def run_model(self, img: np.ndarray, **_) -> Tuple[torch.Tensor, bool]:
#         self.load_model()
#         img = HWC3(img)
#         faces = self.model.get(img)
#         if not faces:
#             raise RuntimeError("Insightface: No face found in image.")
#         if len(faces) > 1:
#             print("Insightface: More than one face is detected in the image. Only the first one will be used.")
...
#                 MeshGraphormerDetector,  # installed via hand_refiner_portable
#             )
