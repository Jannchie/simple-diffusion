...
def retrieve_timesteps(
    ...
    if timesteps is not None:
        ...
        if not accepts_timesteps:
            raise ValueError(f"The current scheduler class {scheduler.__class__}'s `set_timesteps` does not support custom" f" timestep schedules. Please check whether you are using the correct scheduler.")
...
def _images_to_tensors(imgs: List[PIL.Image.Image], width: int, height: int, device: torch.device, dtype: torch.dtype,) -> torch.Tensor:
    buf = []
    for image_ in imgs:
...
def mask_images_to_float_tensor(imgs: List[PIL.Image.Image], resize_wh: Optional[Tuple[int, int]] = None, resample: Optional[PIL.Image.Resampling] = None,) -> torch.Tensor:
...
class ControlNetUnits:
    def __init__(
        self, units: tuple[ControlNetUnit],
    ):
...
class JannchiePipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        **args,
        ...
        if controlnet_units:
            self.controlnet = MultiControlNetModel(controlnets=controlnet_units.controlnets)
        ...
        if self.controlnet:
            ...
            n_controlnet_unit = len(controlnet.nets) if isinstance(controlnet, MultiControlNetModel) else 0
        else:
            n_controlnet_unit = 0
        # 1. Check inputs. Raise error if not correct
        self.check_inputs(
            prompt, controlnet_images, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds, controlnet_conditioning_scale=controlnet_conditioning_scale, control_guidance_start=control_guidance_start, control_guidance_end=control_guidance_end,
        )
        ...
        prompt_embeds = self.encode_prompt(prompt, device, num_images_per_prompt, do_classifier_free_guidance, negative_prompt, prompt_embeds=prompt_embeds, negative_prompt_embeds=negative_prompt_embeds, lora_scale=text_encoder_lora_scale,)
        ...
        if n_controlnet_unit != 0:
            if isinstance(controlnet, ControlNetModel):
                controlnet_images = self.prepare_image(
                    image=controlnet_images, width=width, height=height, batch_size=batch_size * num_images_per_prompt, num_images_per_prompt=num_images_per_prompt, device=device, dtype=controlnet.dtype, do_classifier_free_guidance=do_classifier_free_guidance, guess_mode=guess_mode,
                ).to(device=device)
                ...
                for image_ in controlnet_images:
                    image_ = self.prepare_image(
                        image=image_, width=width, height=height, batch_size=batch_size * num_images_per_prompt, num_images_per_prompt=num_images_per_prompt, device=device, dtype=controlnet.dtype, do_classifier_free_guidance=do_classifier_free_guidance, guess_mode=guess_mode,
                    ).to(device=device)
        ...
        if ref_image is not None:
            if isinstance(ref_image, PIL.Image.Image):
                ref_image = self.image_processor.preprocess(ref_image, height=height, width=width)
        ...
        if image is not None:
            ...
            if isinstance(image, torch.Tensor):
                ...
                input_latents = self.image_to_latents(image, batch_size * num_images_per_prompt, self.unet.dtype, device, generator, False,)  # it will duplicate the latents after this step

        num_channels_unet = self.unet.config.in_channels
        return_image_latents = num_channels_unet == 4
        latents_outputs = self.prepare_latents(batch_size * num_images_per_prompt, num_channels_latents, height, width, self.unet.dtype, device, generator, latents, image, latent_timestep, is_strength_max=strength == 1.0, return_noise=True, return_image_latents=return_image_latents,)
        ...
        if mask_image is not None:
            mask_condition = self.mask_processor.preprocess(mask_image, height=height, width=width).to(device=device)
            init_image = image
            ...
            mask, masked_image_latents = self.prepare_mask_latents(mask_condition, masked_image, batch_size * num_images_per_prompt, height, width, self.unet.dtype, device, generator, do_classifier_free_guidance,)

        # 8. Prepare reference latent variables
        if ref_image is not None:
            ref_image_latents = self.image_to_latents(ref_image, batch_size * num_images_per_prompt, self.unet.dtype, device, generator, do_classifier_free_guidance,)

        # 9. Prepare extra step kwargs. TODO: Logic should ideally just be moved out of the pipeline
        extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)

        if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
            image_embeds = self.prepare_ip_adapter_image_embeds(ip_adapter_image, ip_adapter_image_embeds, device, batch_size * num_images_per_prompt, do_classifier_free_guidance,)

        # Add image embeds for IP-Adapter
        added_cond_kwargs = {"image_embeds": image_embeds} if (ip_adapter_image is not None or ip_adapter_image_embeds is not None) else {}

        # text_embeds for reference, TODO: I forgot why it is needed
        added_cond_kwargs["text_embeds"] = prompt_embeds

        ref_mask_dict, out_mask_dict = self.get_ref_mask_dicts(ref_image_mask, height, width, num_images_per_prompt, write_mask, bool_mask, device, batch_size,)
        ref_data = ReferenceData(
            ref_image=ref_image, ref_image_mask=ref_image_mask, style_fidelity=style_fidelity, attention_auto_machine_weight=attention_auto_machine_weight, gn_auto_machine_weight=gn_auto_machine_weight, ref_mask_dict=ref_mask_dict, out_mask_dict=out_mask_dict, strength=reference_strength,
        )
        if reference_attn:
            self.unet = ReferenceOnlyUNet2DConditionModel.from_unet(self.unet, ref_data, reference_attn, reference_adain,)
        else:
            self.unet = ReferenceOnlyUNet2DConditionModel.revert_unet(self.unet)  # 9. Modify self attention and group norm
        if ref_image is not None:
            self.unet.ref_data.MODE = "write"
            self.unet.ref_data.uc_mask = torch.Tensor([1] * batch_size * num_images_per_prompt + [0] * batch_size * num_images_per_prompt).type_as(ref_image_latents).bool()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if ref_image is not None:
                    ...
                    single_noise = randn_tensor(single_shape, generator=generator, device=device, dtype=ref_image_latents.dtype,)
                    noise_for_ref = single_noise.repeat_interleave(ref_image_latents.shape[0], dim=0)
                    ref_xt = self.scheduler.add_noise(ref_image_latents, noise_for_ref, t.reshape(1,),)
                    # ref_xt = self.scheduler.scale_model_input(ref_xt, t)

                    self.unet.ref_data.MODE = "write"
                    self.unet(
                        ref_xt, t, encoder_hidden_states=prompt_embeds, cross_attention_kwargs=cross_attention_kwargs, return_dict=False, added_cond_kwargs=added_cond_kwargs,
                    )
                ...
                assert isinstance(self.controlnet, (ControlNetModel, MultiControlNetModel))
                if n_controlnet_unit != 0:
                    down_block_res_samples, mid_block_res_sample = self.controlnet(
                        control_model_input.to(device=device, dtype=self.controlnet.dtype), t, encoder_hidden_states=controlnet_prompt_embeds, controlnet_cond=controlnet_images, conditioning_scale=cond_scale, guess_mode=guess_mode, return_dict=False,
                    )
                ...
                noise_pred = self.unet(
                    ...
                    t,
                    encoder_hidden_states=prompt_embeds.to(device=device, dtype=self.unet.dtype),
                ...
                input_latents = self.scheduler.step(noise_pred, t, input_latents, **extra_step_kwargs)["prev_sample"]
                if num_channels_unet == 4 and (mask_image is not None or masked_image_latents is not None):
                    ...
                    if i < len(timesteps) - 1:
                        ...
                        init_latents_proper = init_latents_proper.to(device=device, dtype=self.unet.dtype)

                    input_latents = (1 - init_mask) * init_latents_proper + init_mask * input_latents
        ...
        if output_type != "latent":
            ...
            result_imgs = self.vae.decode(input_latents / self.vae.config.scaling_factor, return_dict=False)[0]
            result_imgs, has_nsfw_concept = self.run_safety_checker(result_imgs, device, prompt_embeds.dtype)
        else:
        ...
        result_imgs = self.image_processor.postprocess(result_imgs, output_type=output_type, do_denormalize=do_denormalize)
        ...
        return StableDiffusionPipelineOutput(images=img_out, nsfw_content_detected=has_nsfw_concept)

    def get_ref_mask_dicts(
        self, ref_image_mask, height, width, num_images_per_prompt, write_mask, bool_mask, device, batch_size,
    ):
        ...
        for i in range(4):
            ...
            if ref_image_mask:
                ...
                tmp_mt_key = mask_images_to_float_tensor([ref_image_mask], resize_wh=resize_wh,).to(device=device, dtype=self.unet.dtype)

                mt_key = tmp_mt_key.flatten() > 0.5 if bool_mask else tmp_mt_key.flatten()
                ref_mask_dict[mt_key.shape[-1]] = mt_key.repeat(batch_size * num_images_per_prompt, 1)

            if write_mask:
                tmp_mt_query = mask_images_to_float_tensor([write_mask], resize_wh=resize_wh,).to(device=device, dtype=self.unet.dtype)
                if bool_mask:
                    mt_query = tmp_mt_query.flatten(1) > 0.5
                else:
                    mt_query = tmp_mt_query.flatten(1)
                out_mask_dict[mt_query.shape[-1]] = mt_query.repeat(batch_size * num_images_per_prompt, 1)
    ...
    def __init__(
        ...
        self.set_progress_bar_config(
            desc=f"Running {pipe_class_name}...", unit_scale=True, bar_format="{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}{postfix}]",
        )
        ...
        super().__init__(
            vae=vae, text_encoder=text_encoder, tokenizer=tokenizer, unet=unet, controlnet=controlnet, scheduler=scheduler, safety_checker=safety_checker, feature_extractor=feature_extractor, image_encoder=image_encoder, requires_safety_checker=requires_safety_checker,
        )
        self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)
        self.mask_processor = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor, do_normalize=False, do_resize=False, do_binarize=True, do_convert_grayscale=True,)

    def image_to_latents(
        self, image, batch_size, dtype, device, generator, do_classifier_free_guidance,
    ):
        image = image.to(device=device, dtype=dtype)

        # encode the mask image into latents space so we can concatenate it to the latents
        if isinstance(generator, list):
            image_latents = [retrieve_latents(self.vae.encode(image[i : i + 1]), generator[i]) for i in range(batch_size)]
    ...
    def get_img_from_latents(self, latents: torch.Tensor):
        ...
        dec_tensor = self.vae.decode(norm_latents / self.vae.config.scaling_factor, return_dict=False)[0]
        dec_images = self.image_processor.postprocess(dec_tensor, output_type="np", do_denormalize=[True] * dec_tensor.shape[0])
        dec_image_zero = dec_images
        dec_image_zero = np.nan_to_num(dec_image_zero)
        image_out_np = np.clip((dec_image_zero * 255.0).round().astype(int), a_min=0, a_max=255).astype(np.uint8)
        return [PIL.Image.fromarray(img) for img in image_out_np]

    def encode_images_to_latents(self, imgs: List[PIL.Image.Image], generator: torch.Generator, device: torch.device, dtype: torch.dtype,) -> torch.Tensor:
        width, height = imgs[0].size
        image_tensor = _images_to_tensors(imgs=imgs, width=width, height=height, device=device, dtype=dtype)
        # encode the mask image into latents space so we can concatenate it to the latents
        if isinstance(generator, list):
            image_latent = torch.cat([retrieve_latents(self.vae.encode(image_tensor[i : i + 1]), generator[i]) for i in range(image_tensor.shape[0])], dim=0,)
        else:
    ...
    def prepare_latents(
        self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None, image=None, timestep=None, is_strength_max=True, return_noise=False, return_image_latents=False,
    ):
        shape = (
            batch_size,
    ...
    def prepare_mask_latents(
        self, mask: torch.Tensor, masked_image: torch.Tensor, batch_size: int, height: int, width: int, dtype: torch.dtype, device: torch.device, generator: torch.Generator, do_classifier_free_guidance: bool,
    ):
...
class ReferenceOnlyUNet2DConditionModel(UNet2DConditionModel):
    @classmethod
    def from_unet(cls, unet: UNet2DConditionModel, ref_data: ReferenceData = ReferenceData(), reference_attn: bool = False, reference_adain: bool = False,) -> "ReferenceOnlyUNet2DConditionModel":
        # 创建一个新的子类实例
        basic_transformer_idx = 0
        ...
        # 计算 attn_weight
        basic_transformer_blocks = sorted(basic_transformer_blocks, key=lambda x: -x.norm1.normalized_shape[0])
    ...
    @classmethod
    def revert_unet(cls, unet: "ReferenceOnlyUNet2DConditionModel") -> UNet2DConditionModel:
...
class BasicTransformerBlockReferenceOnly(BasicTransformerBlock):
    @classmethod
    def from_module(cls, module: BasicTransformerBlock) -> "BasicTransformerBlockReferenceOnly":
        ...
        if self.only_cross_attention:
            ...
            if attention_auto_machine_weight > self.attn_weight:
                ...
                if do_classifier_free_guidance and style_fidelity > 0:
                    ...
                    attn_output_c[uc_mask] = self.attn1(norm_hidden_states[uc_mask], encoder_hidden_states=norm_hidden_states[uc_mask], **cross_attention_kwargs,)
...
class CrossAttnDownBlock2DReferenceOnly(CrossAttnDownBlock2D):
    @classmethod
    def from_module(cls, module: CrossAttnDownBlock2D) -> "CrossAttnDownBlock2DReferenceOnly":
        module.__class__ = cls
        return module
...
class DownBlock2DReferenceOnly(DownBlock2D):
    ...
    def forward(self, hidden_states: torch.FloatTensor, temb: torch.FloatTensor | None = None, *args, **kwargs,) -> Tuple[torch.FloatTensor | Tuple[torch.FloatTensor]]:
...
class UNetMidBlock2DCrossAttnReferenceOnly(UNetMidBlock2DCrossAttn):
    ...
    def forward(
        ...
        return super().forward(hidden_states, temb, encoder_hidden_states, attention_mask, cross_attention_kwargs, encoder_attention_mask,)
...
class UpBlock2DReferenceOnly(UpBlock2D):
    ...
    def forward(self, hidden_states: torch.FloatTensor, res_hidden_states_tuple: Tuple[torch.FloatTensor, ...], temb: Optional[torch.FloatTensor] = None, upsample_size: Optional[int] = None, *args, **kwargs,) -> torch.FloatTensor:
        MODE = self.ref_data.MODE
        gn_auto_machine_weight = self.ref_data.gn_auto_machine_weight
        do_classifier_free_guidance = self.ref_data.do_classifier_free_guidance
        style_fidelity = self.ref_data.style_fidelity
