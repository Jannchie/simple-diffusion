...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    if p.refiner_checkpoint not in (None, "", "None", "none"):
        ...
        if p.refiner_checkpoint_info is None:
            raise RuntimeError(f"Could not find checkpoint with name {p.refiner_checkpoint}")
    ...
    with torch.inference_mode():
        ...
        for n in range(p.n_iter):
            ...
            for hook in p.hooks:
                hook.process_batch(p, batch_number=n, prompts=p.prompts, seeds=p.seeds, subseeds=p.subseeds)
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        if self.firstpass_image is not None and self.enable_hr:
            ...
            for hook in self.hooks:
                hook.process_before_every_sampling(self, x=x, noise=x, c=conditioning, uc=unconditional_conditioning)
    ...
    def sample_hr_pass(self, samples, decoded_samples, seeds, subseeds, subseed_strength, prompts):
        ...
        for hook in self.hooks:
            hook.process_before_every_sampling(self, x=samples, noise=noise, c=self.hr_c, uc=self.hr_uc)
