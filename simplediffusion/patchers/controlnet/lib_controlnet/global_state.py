...
def traverse_all_files(curr_path, model_list):
    f_list = [(os.path.join(curr_path, entry.name), entry.stat()) for entry in os.scandir(curr_path) if os.path.isdir(curr_path)]
...
def get_filtered_controlnet_names(tag):
    ...
    return [x for x in controlnet_names if x == "None" or (any(f.lower() in x.lower() for f in model_filename_filters) and get_sd_version().is_compatible_with(StableDiffusionVersion.detect_from_model_name(x)))]
...
def update_controlnet_filenames():
    ...
    for path in paths:
        ...
        controlnet_filename_dict |= found
