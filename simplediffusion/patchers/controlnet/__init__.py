...
from modules import images, masking, shared
...
class ControlNetHook(Hook):
    ...
    def get_enabled_units(self) -> list[ControlNetUnit | dict]:
        ...
        units = [ControlNetUnit.from_dict(unit) if isinstance(unit, dict) else unit for unit in self.units]
        assert all(isinstance(unit, ControlNetUnit) for unit in units)
        return [x for x in units if x.enabled]

    def try_crop_image_with_a1111_mask(self, p: StableDiffusionProcessing, preprocessor, input_image: np.ndarray, resize_mode: external_code.ResizeMode) -> np.ndarray:
    ...
    def get_input_data(
        ...
        if unit.input_mode == external_code.InputMode.MERGE:
            ...
            image = self.try_crop_image_with_a1111_mask(p, unit.preprocessor, image, resize_mode)

            if mask is not None:
                mask = cv2.resize(HWC3(mask), (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST)
                mask = self.try_crop_image_with_a1111_mask(p, unit.preprocessor, mask, resize_mode)

            image_list = [[image, mask]]

        if resize_mode == external_code.ResizeMode.OUTER_FIT and unit.preprocessor.expand_mask_when_resize_and_fill:
            new_image_list = []
    ...
    def get_target_dimensions(p: StableDiffusionProcessing) -> Tuple[int, int, int, int]:
        ...
        if high_res_fix := isinstance(p, StableDiffusionProcessingTxt2Img) and getattr(p, "enable_hr", False):
    ...
    @torch.no_grad()
    def process_unit(self, p: StableDiffusionProcessing, unit: ControlNetUnit, params: ControlNetCachedParameters, *args, **kwargs):
        ...
        for input_image, input_mask in optional_tqdm(input_list, len(input_list) > 1):
            ...
            # 将图片导出到 ./controlnet_condition.png
    ...
    def process_unit_before_every_sampling(self, p: StableDiffusionProcessing, unit: ControlNetUnit, params: ControlNetCachedParameters, *args, **kwargs):
        ...
        soft_weighting = {
            "input": [0.09941396206337118, 0.12050177219802567, 0.14606275417942507, 0.17704576264172736, 0.214600924414215, 0.26012233262329093, 0.3152997971191405, 0.3821815722656249, 0.4632503906249999, 0.561515625, 0.6806249999999999, 0.825],
            "middle": [0.561515625] if p.sd_model.is_sdxl else [1.0],
            "output": [0.09941396206337118, 0.12050177219802567, 0.14606275417942507, 0.17704576264172736, 0.214600924414215, 0.26012233262329093, 0.3152997971191405, 0.3821815722656249, 0.4632503906249999, 0.561515625, 0.6806249999999999, 0.825],
        }
    ...
    def bound_check_params(self, unit: ControlNetUnit) -> None:
        ...
        if unit.processor_res < 0:
            unit.processor_res = int(unit.preprocessor.slider_resolution.gradio_update_kwargs.get("value", 512))

        if unit.threshold_a < 0:
            unit.threshold_a = int(unit.preprocessor.slider_1.gradio_update_kwargs.get("value", 1.0))

        if unit.threshold_b < 0:
            unit.threshold_b = int(unit.preprocessor.slider_2.gradio_update_kwargs.get("value", 1.0))
