...
class DiffusionEngine(pl.LightningModule):
    ...
    @torch.no_grad()
    def sample(
        self, cond: Dict, uc: Union[Dict, None] = None, batch_size: int = 16, shape: Union[None, Tuple, List] = None, **kwargs,
    ):
    ...
    def log_images(self, batch: Dict, N: int = 8, sample: bool = True, ucg_keys: List[str] = None, **kwargs,) -> Dict:
        ...
        if ucg_keys:
            assert all(map(lambda x: x in conditioner_input_keys, ucg_keys)), "Each defined ucg key for sampling must be in the provided conditioner input keys," f"but we have {ucg_keys} vs. {conditioner_input_keys}"
        else:
        ...
        c, uc = self.conditioner.get_unconditional_conditioning(batch, force_uc_zero_embeddings=ucg_keys if len(self.conditioner.embedders) > 0 else [],)
