...
def load_state_dict(state_dict) -> PyTorchModel:
    ...
    # Swift-SRGAN
    elif "model" in state_dict_keys and "initial.cnn.depthwise.weight" in state_dict["model"].keys():
        model = SwiftSRGAN(state_dict)
    # SwinIR, Swin2SR, HAT
    elif "layers.0.residual_group.blocks.0.norm1.weight" in state_dict_keys:
        if "layers.0.residual_group.blocks.0.conv_block.cab.0.weight" in state_dict_keys:
    ...
    # GFPGAN
    elif "toRGB.0.weight" in state_dict_keys and "stylegan_decoder.style_mlp.1.weight" in state_dict_keys:
        model = GFPGANv1Clean(state_dict)
    # RestoreFormer
    elif "encoder.conv_in.weight" in state_dict_keys and "encoder.down.0.block.0.norm1.weight" in state_dict_keys:
        model = RestoreFormer(state_dict)
    elif "encoder.blocks.0.weight" in state_dict_keys and "quantize.embedding.weight" in state_dict_keys:
        model = CodeFormer(state_dict)
    # LaMa
    elif "model.model.1.bn_l.running_mean" in state_dict_keys or "generator.model.1.bn_l.running_mean" in state_dict_keys:
