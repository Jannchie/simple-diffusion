...
class OmniSR(nn.Module):
    def __init__(
        ...
        if "residual_layer.0.residual_layer.0.layer.2.fn.rel_pos_bias.weight" in state_dict.keys():
            rel_pos_bias_weight = state_dict["residual_layer.0.residual_layer.0.layer.2.fn.rel_pos_bias.weight"].shape[0]
        ...
        for _ in range(res_num):
            temp_res = OSAG(channel_num=num_feat, bias=bias, block_num=block_num, ffn_bias=ffn_bias, window_size=self.window_size, pe=pe,)
