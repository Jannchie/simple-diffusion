...
class SeperableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, bias=True):
        ...
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, groups=in_channels, bias=bias, padding=padding,)
...
class ConvBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, use_act=True, use_bn=True, discriminator=False, **kwargs,
    ):
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        self.num_blocks = len(set([x.split(".")[1] for x in self.state.keys() if "residual" in x]))
        self.scale: int = 2 ** len(set([x.split(".")[1] for x in self.state.keys() if "upsampler" in x]))
