...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        return (
            z_q,
            loss,
            {"perplexity": perplexity, "min_encodings": min_encodings, "min_encoding_indices": min_encoding_indices, "min_encoding_scores": min_encoding_scores, "mean_distance": mean_distance,},
        )
...
class Encoder(nn.Module):
    def __init__(
        self, in_channels, nf, out_channels, ch_mult, num_res_blocks, resolution, attn_resolutions,
    ):
        super().__init__()
        self.nf = nf
...
class VQAutoEncoder(nn.Module):
    def __init__(
        self, img_size, nf, ch_mult, quantizer="nearest", res_blocks=2, attn_resolutions=[16], codebook_size=1024, emb_dim=256, beta=0.25, gumbel_straight_through=False, gumbel_kl_weight=1e-8, model_path=None,
    ):
        super().__init__()
        self.in_channels = 3
        self.nf = nf
...
class CodeFormer(VQAutoEncoder):
    def __init__(self, state_dict):
        ...
        try:
            n_layers = len(set([x.split(".")[1] for x in state_dict.keys() if "ft_layers" in x]))
        except:
            pass
