...
class SummarizeChunk(Protocol):
    @staticmethod
    def __call__(query: Tensor, key_t: Tensor, value: Tensor,) -> AttnChunk:
        ...


class ComputeQueryChunkAttn(Protocol):
    @staticmethod
    def __call__(query: Tensor, key_t: Tensor, value: Tensor,) -> Tensor:
        ...
...
def efficient_dot_product_attention(
    query: Tensor, key_t: Tensor, value: Tensor, query_chunk_size=1024, kv_chunk_size: Optional[int] = None, kv_chunk_size_min: Optional[int] = None, use_checkpoint=True, upcast_attention=False, mask=None,
):
    ...
    compute_query_chunk_attn: ComputeQueryChunkAttn = partial(_get_attention_scores_no_kv_chunking, scale=scale, upcast_attention=upcast_attention) if k_tokens <= kv_chunk_size else (
