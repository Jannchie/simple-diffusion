...
class UniPC:
    def __init__(
        self, model_fn, noise_schedule, predict_x0=True, thresholding=False, max_val=1.0, variant="bh1", noise_mask=None, masked_image=None, noise=None,
    ):
        """Construct a UniPC. 
    ...
    def sample(self, x, timesteps, t_start=None, t_end=None, order=3, skip_type="time_uniform", method="singlestep", lower_order_final=True, denoise_to_zero=False, solver_type="dpm_solver", atol=0.0078, rtol=0.05, corrector=False, callback=None, disable_pbar=False):
        ...
        if method == "multistep":
            ...
            for step_index in trange(steps, disable=disable_pbar):
                if self.noise_mask is not None:
                    x = x * self.noise_mask + (1.0 - self.noise_mask) * (self.masked_image * self.noise_schedule.marginal_alpha(timesteps[step_index]) + self.noise * self.noise_schedule.marginal_std(timesteps[step_index]))
                if step_index == 0:
...
def sample_unipc(model, noise, image, sigmas, max_denoise, extra_args=None, callback=None, disable=False, noise_mask=None, variant="bh1"):
    ...
    model_fn = model_wrapper(lambda input, sigma, **kwargs: predict_eps_sigma(model, input, sigma, **kwargs), ns, model_type=model_type, guidance_type="uncond", model_kwargs=extra_args,)
