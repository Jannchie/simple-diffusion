...
class UiSettings:
    ...
    def create_ui(self, loadsave, dummy_component):
        ...
        with gr.Blocks(analytics_enabled=False) as settings_interface:
            ...
            unload_sd_model.click(fn=call_func_and_return_text(sd_models.unload_model_weights, "Unloaded the checkpoint"), inputs=[], outputs=[self.result])

            reload_sd_model.click(fn=call_func_and_return_text(lambda: sd_models.send_model_to_device(shared.sd_model), "Loaded the checkpoint"), inputs=[], outputs=[self.result])
            ...
            restart_gradio.click(
                fn=shared.state.request_restart, _js="restart_reload", inputs=[], outputs=[],
            )
            ...
            sysinfo_check_file.change(
                fn=check_file, inputs=[sysinfo_check_file], outputs=[sysinfo_check_output],
            )
            ...
            calculate_all_checkpoint_hash.click(
                fn=calculate_all_checkpoint_hash_fn, inputs=[calculate_all_checkpoint_hash_threads],
            )
    ...
    def add_functionality(self, demo):
        self.submit.click(
            fn=wrap_gradio_call(lambda *args: self.run_settings(*args), extra_outputs=[gr.update()]), inputs=self.components, outputs=[self.text_settings, self.result],
        )
        ...
        for _i, k, _item in self.quicksettings_list:
            ...
            for method in methods:
                handler = method(fn=lambda value, k=k: self.run_settings_single(value, key=k), inputs=[component], outputs=[component, self.text_settings], show_progress=False,)
                script_callbacks.setting_updated_event_subscriber_chain(
                    handler=handler, component=component, setting_name=k,
                )
        ...
        script_callbacks.setting_updated_event_subscriber_chain(handler=handler, component=self.component_dict["sd_model_checkpoint"], setting_name="sd_model_checkpoint")
        ...
        demo.load(
            fn=get_settings_values, inputs=[], outputs=[self.component_dict[k] for k in component_keys], queue=False,
        )
