...
class Git(git.Git):
    ...
    def get_object_header(self, ref: str | bytes) -> tuple[str, str, int]:
        ret = subprocess.check_output([self.GIT_PYTHON_GIT_EXECUTABLE, "cat-file", "--batch-check"], input=self._prepare_ref(ref), cwd=self._working_dir, timeout=2,)
    ...
    def stream_object_data(self, ref: str) -> tuple[str, str, int, Git.CatFileContentStream]:
        ...
        ret = subprocess.check_output([self.GIT_PYTHON_GIT_EXECUTABLE, "cat-file", "--batch"], input=self._prepare_ref(ref), cwd=self._working_dir, timeout=30,)
        bio = io.BytesIO(ret)
